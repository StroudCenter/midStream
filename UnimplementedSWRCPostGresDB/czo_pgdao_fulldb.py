...
class czoDao(BaseDao):
    ...
    def get_variable_by_code(self, var_code):
        ...
        var1.LoggerID = row[10]
        var1.LoggerCode = row[11]
        var1.DeploymentDateTime = row[12]
        var1.DeploymentUTCOffset = row[13]
        #var1.ValueType = None
            #Leaving empty assumes that the value type is "Feild Observation" from czo_model
        #var1.IsRegular = None
            #Leaving empty assumes that the IsRegular=TRUE from czo_model
    ...
    def get_series_by_sitecode(self, site_code):
        ...
        # open database, read all of the variable codes available for this site
        conn = pg.connect(DB_CONNECT_STR)
        cur = conn.cursor()    
        cur.execute("""
        SELECT DISTINCT
    ...
    def parse_date_strings(self, begin_date_time_string, end_date_time_string):
        ...
        try:
            if begin_date_time_string:
                ...
                if DEBUG_PRINT:
                    print '- no given start, using default of -', b
        except:
        ...
        try:
            if end_date_time_string:
                ...
                if DEBUG_PRINT:
                    print '- no given end, using current time -', e
        except:
...
#            print '*create_datavalue_from_row: ' + repr(row) + ' - ' + repr(value_index)        
    ...
    def get_datavalues(self, site_code, var_code, begin_date_time=None,
        ...
        # This query is identical to the get_series_by_sitecode query.
        cur.execute("""
        SELECT DISTINCT
        	concat(DataSeries.sensorid,dataseries.variableid,dataseries.mediumtypeid,dataseries.datatypeid,dataseries.variableunitsid) AS variablecode,
        	Sites.SiteNameCode
        ...
        value_index = varResult.csvColumn + 2
        ...
        if DEBUG_PRINT:
            t1 = time.time() #Calculate the time the request takes.
        ...
        data_file_list.append("data_longformat_test/historic.csv") # first a historic data file
        ...
        if DEBUG_PRINT:
            ...
            print 'searching datafiles for: '   
            print '- Logger ID' , varResult.LoggerID  
            print '- Logger Code' , varResult.LoggerCode  
            print '- Data Column' , varResult.csvColumn
            print '- Values after deployment on' , varResult.DeploymentDateTime
        ...
        for data_file in data_file_list:        
            try:
                with open(data_file, 'rb') as f:
                    reader = csv.reader(f)
                    if DEBUG_PRINT:                
                        print 'now searching in : ' + data_file
                    for row in reader:
                        if len(row) >= 6: # error check to be sure row has at least 6 elements
                            if row[2].strip() == varResult.LoggerCode: # first check if this row is for the requested logger
                                try: # catch a bad time string - though these should already be eliminated in the receiver
                                    value_date = parse(row[0]).replace(tzinfo=tz(None, int(row[1].strip())*3600))
                                    #if DEBUG_PRINT:
                                    #    print value_date                                
                ...
                pass
                if DEBUG_PRINT:                
                    print 'file not found or contains no applicable data: ' + data_file
